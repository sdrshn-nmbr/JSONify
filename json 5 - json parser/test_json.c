...
static int _test_get_element_simple()
{
    mu_start();
    // ──────────────────────────────────────────────────────
    BSTNode *root = NULL;
    _bst_insert(&root, "Ananth", (Element){.type = ELEMENT_INT, .as_int = 1});
    _bst_insert(&root, "Bobby", (Element){.type = ELEMENT_INT, .as_int = 2});
    _bst_insert(&root, "AJ", (Element){.type = ELEMENT_INT, .as_int = 3});
    Element *a_element = get_element(root, "AJ");
    mu_check(a_element != NULL);
    mu_check(a_element->type == ELEMENT_INT);
    mu_check(a_element->as_int == 3);
    free_element(*a_element);
    free_element((Element){.type = ELEMENT_OBJECT, .as_object = root});
    // ──────────────────────────────────────────────────────
    mu_end();
}

static int _test_get_element_returns_null()
{
    mu_start();
    // ──────────────────────────────────────────────────────
    BSTNode *root = NULL;
    ...
    _bst_insert(&root, "Bobby", (Element){.type = ELEMENT_INT, .as_int = 2});
    _bst_insert(&root, "AJ", (Element){.type = ELEMENT_INT, .as_int = 3});
    _bst_insert(&root, "AJ", (Element){.type = ELEMENT_INT, .as_int = 1});
    Element *a_element = get_element(root, "AJJ");
    mu_check(a_element == NULL);
    free_element((Element){.type = ELEMENT_OBJECT, .as_object = root});
    // ──────────────────────────────────────────────────────
    mu_end();
...
int main(int argc, char *argv[])
{
    ...
    mu_run(_test_get_element_simple);
    mu_run(_test_get_element_returns_null);
