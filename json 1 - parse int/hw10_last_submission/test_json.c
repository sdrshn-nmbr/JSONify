...
static int _test_parse_int_full_string()
{
	...
	bool is_success = parse_int(&result, &pos);
	mu_check(!is_success);  // because the input is valid
	mu_check(pos == input); // failure should be at the first character in the input, not the negative sign
	// ──────────────────────────────────────────────────────
	mu_end();
}
...
static int _test_parse_int_full_number()
{
	...
	bool is_success = parse_int(&result, &pos);
	mu_check(is_success);       // because the input is valid
	mu_check(pos == input + 9); // failure should be at the first character in the input, not the negative sign
...
static int _test_parse_int_number_with_string()
{
	...
	bool is_success = parse_int(&result, &pos);
	mu_check(is_success);       // because the input is valid
	mu_check(pos == input + 3); // failure should be at the first character in the input, not the negative sign
	mu_check(result == 123);
	// ──────────────────────────────────────────────────────
	mu_end();
}
...
static int _test_parse_int_negative_number_with_string()
{
	...
	bool is_success = parse_int(&result, &pos);
	mu_check(is_success);       // because the input is valid
	mu_check(pos == input + 4); // failure should be at the first character in the input, not the negative sign
	mu_check(result == -123);
	// ──────────────────────────────────────────────────────
	mu_end();
}
